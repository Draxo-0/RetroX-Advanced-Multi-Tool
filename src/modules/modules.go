...
func (m *Modules) Input(text string) string {
	...
	fmt.Printf("%s", text) // Align prompt to the left
	scanner.Scan()
	return scanner.Text()
}

func (m *Modules) InputBool(text string) bool {
	return m.Input(text+" y/n: ") == "y"
}
...
func (m *Modules) InputInt(text string) int {
	...
	if err != nil {
		...
		var discard string
		fmt.Scanln(&discard)
		return -1 // Return -1 for invalid input instead of 0
	}
	return d
}
...
// FetchInputData read thefiles needed for launching RetroX
...
func (m *Modules) HalfToken(T string, v int) string {
	...
	if len(parts) <= v {
		// If token doesn't have enough parts, return a safe fallback
		if len(T) > 8 {
			return T[:8] + "..."
		}
		return T
...
func StoreVoiceConnection(token string, vc *VoiceConnection) {
	voiceManager.mu.Lock()
	defer voiceManager.mu.Unlock()
	voiceManager.Connections[token] = vc
}

func GetVoiceConnection(token string) *VoiceConnection {
	voiceManager.mu.RLock()
	defer voiceManager.mu.RUnlock()
	return voiceManager.Connections[token]
}

func RemoveVoiceConnection(token string) {
	voiceManager.mu.Lock()
	defer voiceManager.mu.Unlock()
	delete(voiceManager.Connections, token)
}

func GetAllVoiceConnections() map[string]*VoiceConnection {
	voiceManager.mu.RLock()
	defer voiceManager.mu.RUnlock()
	connections := make(map[string]*VoiceConnection)
	for k, v := range voiceManager.Connections {
		connections[k] = v
	}
	return connections
}

func (m *Modules) getTerminalWidth() int {
	// Use a reasonable default that works well for most terminals
	return 120
}
