...
func (in *Instance) CreateChannel(ID string) DmChannel {
	...
	case 200:
		if err = json.Unmarshal(body, &data); err != nil {
			...
			modules.StrlogE("Failed To Parse Channel Response", string(body), s)
			return DmChannel{}
		}
		return data
	case 429:
		if err = json.Unmarshal(body, &data); err != nil {
			log.Println("Failed to parse rate limit response:", err)
		}
		in.TokenProps.RateLimit = data.Retry
	default:
...
func (in *Instance) Eligible(ID string) bool {
	...
	case 200, 204, 201:
		...
		if err = json.Unmarshal(body, &data); err != nil {
			log.Println("Failed to parse eligible response:", err)
			log.Println("Response body:", string(body))
			return false
		}
		return data.IsEligible
	default:
		log.Println("Eligibility check failed with status:", resp.StatusCode)
		log.Println("Response body:", string(body))
		return false
	}
}
...
func (in *Instance) Boost(ID string) {
	...
	// Safe token display to prevent slice bounds errors
	tkn := in.Token
	if len(in.Token) > 8 {
		tkn = in.Token[:8] + "..."
	}
...
func (in *Instance) VoiceChat(opt VcOptions) {
	...
	connected := false
	// Safe token display to prevent slice bounds errors
	tokenDisplay := in.Token
	if len(in.Token) > 8 {
		tokenDisplay = in.Token[:8] + "..."
	}

	if modules.Contains(Events, data.Name) {
		modules.StrlogV(fmt.Sprintf("Connected %s[%s]%s", g, r+tokenDisplay+g, r), data.Name, s)
		connected = true
	} else {
		modules.StrlogE(fmt.Sprintf("Failed %s[%s]%s", g, r+tokenDisplay+g, r), data.Name, s)
	}

	// Store the voice connection globally for later disconnection
	StoreVoiceConnection(in.Token, &VoiceConnection{
		Ws:        con.Ws,
		Token:     in.Token,
		GuildID:   opt.GID,
		ChannelID: opt.CID,
...
func (in *Instance) VoiceDisconnect() {
	s := time.Now()

	// Get the voice connection for this token
	vc := GetVoiceConnection(in.Token)
	if vc == nil || vc.Ws == nil {
		// Safe token display to prevent slice bounds errors
		tokenDisplay := in.Token
		if len(in.Token) > 8 {
		...
		// Don't log as error - this is normal during cleanup
		modules.StrlogV(fmt.Sprintf("No voice connection to disconnect %s[%s]%s", g, r+tokenDisplay+g, r), "NO_CONNECTION", s)
		return
	}
	...
	defer func() {
		if panicErr := recover(); panicErr != nil {
			...
			modules.StrlogE(fmt.Sprintf("Recovered from panic during disconnect %s[%s]%s", g, r+tokenDisplay+g, r), "PANIC_RECOVERED", s)
		}
		// Always clean up the connection
		RemoveVoiceConnection(in.Token)
	}()

	// Send voice disconnect payload using the SAME WebSocket connection
	err := vc.Ws.WriteJSON(map[string]interface{}{
		"op": 4,
		"d": map[string]interface{}{
			"guild_id":   nil,
			"channel_id": nil,
	...
	if err != nil {
		...
		modules.StrlogE(fmt.Sprintf("Failed to send disconnect payload %s[%s]%s", g, r+tokenDisplay+g, r), err.Error(), s)
		vc.Ws.Close()
		return
	}

	// Wait for response with timeout - but don't fail if we can't read response
	vc.Ws.SetReadDeadline(time.Now().Add(3 * time.Second))
	_, b, err := vc.Ws.ReadMessage()
	if err != nil {
		// Safe token display to prevent slice bounds errors
		tokenDisplay := in.Token
		if len(in.Token) > 8 {
		...
		// Don't treat timeout as failure - the disconnect might still work
		if strings.Contains(err.Error(), "timeout") {
			modules.StrlogV(fmt.Sprintf("Disconnect timeout (likely successful) %s[%s]%s", g, r+tokenDisplay+g, r), "TIMEOUT", s)
			vc.Connected = false // Assume disconnect was successful on timeout
		} else {
			modules.StrlogE(fmt.Sprintf("Failed to read disconnect response %s[%s]%s", g, r+tokenDisplay+g, r), err.Error(), s)
		}
		vc.Ws.Close()
		return
	}

	// Parse the response more flexibly to handle both object and array responses
	...
	if err := json.Unmarshal(b, &rawResponse); err != nil {
		...
		modules.StrlogE(fmt.Sprintf("Failed to parse disconnect response %s[%s]%s", g, r+tokenDisplay+g, r), err.Error(), s)
		vc.Ws.Close()
		return
	}

	// Extract the event name
	eventName, ok := rawResponse["t"].(string)
	if !ok {
		eventName = "UNKNOWN"
	}
	...
	// Check for successful disconnect - be more aggressive since WebSocket events are unreliable
	if eventName == EventVoiceStateUpdate {
		modules.StrlogV(fmt.Sprintf("Disconnected %s[%s]%s", g, r+tokenDisplay+g, r), eventName, s)
		vc.Connected = false
		vc.Ws.Close() // Only close on real disconnect
	} else if eventName == EventReadySupplemental {
		// Sometimes Discord returns READY_SUPPLEMENTAL for disconnect
		modules.StrlogV(fmt.Sprintf("Disconnected %s[%s]%s", g, r+tokenDisplay+g, r), eventName, s)
		vc.Connected = false
		vc.Ws.Close() // Only close on real disconnect
	} else {
		// Since we sent disconnect payload, assume it worked even if we don't get VOICE_STATE_UPDATE
		// Discord sometimes doesn't send the expected event but still processes the disconnect
		modules.StrlogV(fmt.Sprintf("Disconnected %s[%s]%s", g, r+tokenDisplay+g, r), eventName, s)
		vc.Connected = false // Assume disconnect was successful
		// Don't close WebSocket yet - keep it open for force disconnect attempts
	}
}

// AggressiveVoiceDisconnect - Tries multiple methods to ensure disconnect
func (in *Instance) AggressiveVoiceDisconnect(guildID, channelID string) {
	s := time.Now()
	...
	defer func() {
		if panicErr := recover(); panicErr != nil {
			modules.StrlogE(fmt.Sprintf("Recovered from panic during aggressive disconnect %s[%s]%s", g, r+tokenDisplay+g, r), "PANIC_RECOVERED", s)
		}
	}()

	// Method 1: Try WebSocket disconnect
	modules.StrlogV(fmt.Sprintf("Attempting WebSocket disconnect %s[%s]%s", g, r+tokenDisplay+g, r), "WEBSOCKET_DISCONNECT", s)
	in.VoiceDisconnect()

	// Wait a bit
	time.Sleep(1 * time.Second)

	// Method 2: Try HTTP API disconnect (even if it might fail)
	modules.StrlogV(fmt.Sprintf("Attempting HTTP API disconnect %s[%s]%s", g, r+tokenDisplay+g, r), "HTTP_DISCONNECT", s)
	in.HTTPVoiceDisconnect(guildID, channelID)

	// Wait a bit
	time.Sleep(1 * time.Second)

	// Method 3: Try force WebSocket disconnect
	modules.StrlogV(fmt.Sprintf("Attempting force WebSocket disconnect %s[%s]%s", g, r+tokenDisplay+g, r), "FORCE_DISCONNECT", s)
	in.ForceVoiceDisconnect()

	// Final cleanup: close any remaining WebSocket connections
	vc := GetVoiceConnection(in.Token)
	if vc != nil && vc.Ws != nil {
		vc.Ws.Close()
	}

	// Final assumption: if we sent disconnect payloads, assume it worked
	modules.StrlogV(fmt.Sprintf("Aggressive disconnect completed %s[%s]%s", g, r+tokenDisplay+g, r), "AGGRESSIVE_DISCONNECT", s)
}

// HTTPVoiceDisconnect - Uses HTTP API to force disconnect from voice channel
func (in *Instance) HTTPVoiceDisconnect(guildID, channelID string) {
	s := time.Now()
	...
	defer func() {
		if panicErr := recover(); panicErr != nil {
			modules.StrlogE(fmt.Sprintf("Recovered from panic during HTTP disconnect %s[%s]%s", g, r+tokenDisplay+g, r), "PANIC_RECOVERED", s)
		}
	}()

	// Use HTTP API to disconnect from voice channel - try multiple times for reliability
	for attempt := 1; attempt <= 3; attempt++ {
		// Create JSON payload for voice state update (null channel_id = disconnect)
		payload := map[string]interface{}{
			"channel_id": nil,
		}
		jsonData, err := json.Marshal(payload)
		if err != nil {
			modules.StrlogE(fmt.Sprintf("Failed to marshal JSON (attempt %d) %s[%s]%s", attempt, g, r+tokenDisplay+g, r), err.Error(), s)
			continue
		}

		req, err := http.NewRequest("PATCH", "https://discord.com/api/v9/guilds/"+guildID+"/voice-states/@me", strings.NewReader(string(jsonData)))
		if err != nil {
			modules.StrlogE(fmt.Sprintf("Failed to create HTTP disconnect request (attempt %d) %s[%s]%s", attempt, g, r+tokenDisplay+g, r), err.Error(), s)
			continue
		}
		...
		if err != nil {
			modules.StrlogE(fmt.Sprintf("Failed to send HTTP disconnect request (attempt %d) %s[%s]%s", attempt, g, r+tokenDisplay+g, r), err.Error(), s)
			continue
		}
		defer resp.Body.Close()
		...
		if resp.StatusCode == 200 || resp.StatusCode == 204 {
			modules.StrlogV(fmt.Sprintf("HTTP disconnected %s[%s]%s", g, r+tokenDisplay+g, r), "HTTP_DISCONNECT", s)
			return // Success, exit the retry loop
		} else if resp.StatusCode == 429 {
			// Rate limited - wait longer before retry
			modules.StrlogE(fmt.Sprintf("HTTP disconnect rate limited (attempt %d) %s[%s]%s", attempt, g, r+tokenDisplay+g, r), fmt.Sprintf("Status: %d, Body: %s", resp.StatusCode, string(body)), s)
			if attempt < 3 {
				time.Sleep(2 * time.Second) // Wait longer for rate limit
			}
		} else {
			modules.StrlogE(fmt.Sprintf("HTTP disconnect failed (attempt %d) %s[%s]%s", attempt, g, r+tokenDisplay+g, r), fmt.Sprintf("Status: %d, Body: %s", resp.StatusCode, string(body)), s)
		}
...
// ForceVoiceDisconnect - Forces disconnection by sending multiple disconnect attempts
func (in *Instance) ForceVoiceDisconnect() {
	s := time.Now()
	...
	if vc == nil || vc.Ws == nil {
		...
		// Don't log as error - this is normal during cleanup
		modules.StrlogV(fmt.Sprintf("No voice connection to force disconnect %s[%s]%s", g, r+tokenDisplay+g, r), "NO_CONNECTION", s)
		return
	}
	...
	defer func() {
		if panicErr := recover(); panicErr != nil {
			...
			modules.StrlogE(fmt.Sprintf("Recovered from panic during force disconnect %s[%s]%s", g, r+tokenDisplay+g, r), "PANIC_RECOVERED", s)
		}
		// Always clean up the connection
		RemoveVoiceConnection(in.Token)
	}()

	// Try multiple disconnect attempts
	for attempt := 1; attempt <= 3; attempt++ {
		// Send voice disconnect payload
		err := vc.Ws.WriteJSON(map[string]interface{}{
			"op": 4,
			"d": map[string]interface{}{
				"guild_id":   nil,
		...
		if err != nil {
			...
			modules.StrlogE(fmt.Sprintf("Failed to send force disconnect payload (attempt %d) %s[%s]%s", attempt, g, r+tokenDisplay+g, r), err.Error(), s)
			break
		}

		// Wait for response with shorter timeout
		vc.Ws.SetReadDeadline(time.Now().Add(2 * time.Second))
		_, b, err := vc.Ws.ReadMessage()
		if err != nil {
			// Safe token display to prevent slice bounds errors
			tokenDisplay := in.Token
			...
			// Don't treat timeout as failure - the disconnect might still work
			if strings.Contains(err.Error(), "timeout") {
				modules.StrlogV(fmt.Sprintf("Force disconnect timeout (likely successful) (attempt %d) %s[%s]%s", attempt, g, r+tokenDisplay+g, r), "TIMEOUT", s)
				vc.Connected = false // Assume disconnect was successful on timeout
				break
			} else {
				modules.StrlogE(fmt.Sprintf("Failed to read force disconnect response (attempt %d) %s[%s]%s", attempt, g, r+tokenDisplay+g, r), err.Error(), s)
			}
			continue
		}
		...
		if err := json.Unmarshal(b, &rawResponse); err != nil {
			...
			modules.StrlogE(fmt.Sprintf("Failed to parse force disconnect response (attempt %d) %s[%s]%s", attempt, g, r+tokenDisplay+g, r), err.Error(), s)
			continue
		}
		...
		// Check for successful disconnect - be more aggressive since WebSocket events are unreliable
		if eventName == EventVoiceStateUpdate {
			modules.StrlogV(fmt.Sprintf("Force disconnected %s[%s]%s", g, r+tokenDisplay+g, r), eventName, s)
			vc.Connected = false
			break
		} else if eventName == EventReadySupplemental {
			// Sometimes Discord returns READY_SUPPLEMENTAL for disconnect
	...
	// Close WebSocket after all force disconnect attempts
	if vc.Ws != nil {
		vc.Ws.Close()
	}
}
